...
class Replicate(LLM):
    ...
    """Set server usage (for model preparation, leave field empty if don't running on Replicate server)"""
    ...
    def _create_prediction(self, prompt: str, **kwargs: Any) -> Prediction:
        ...
        if self.version_obj is None:
            ...
            if self.use_server:
                server_name = get_from_env('replicate_server_path', 'REPLICATE_SERVER_PATH')
                model = replicate_python.deployments.get(server_name)
            else:
                model = replicate_python.models.get(model_str)
            if self.use_server:
                self.version_obj = replicate_python.models.get(model_str).versions.get(version_str)
            else:
        ...
        if self.use_server:
            return model.predictions.create(
                input=input_
            )
